...
## v0.7.0 Word Timing, Source Units, Context, And Scheduling, Refined In v0.7.3

The v0.7.0 Faster-Whisper path requests per-word timestamps and confidence. v0.7.3 keeps safe words and forward extensions immediate while bounding each inference input at the last voiced callback plus a 350 ms endpoint-analysis tail and preventing a result that began after the recent-voice grace period from replacing the last speech-backed wording. Silero VAD removes that tail before decoding with a 160 ms end-of-speech decision and only 100 ms of acoustic padding. Both backends use word timestamps, a 0.5-second hallucination-silence threshold, and the same high no-speech, low-average-log-probability, and pathological-output checks. A weak final word within the 0.32-second captured-audio edge is hidden until a later decode agrees **and new speech has advanced the acoustic boundary**; repeatedly processing unchanged audio during a pause is not confirmation. The cue ledger still revises one stable `cue_id`; confirmed text seals at punctuation, 14 words, five seconds, audio-window advancement, or a real final. Standard Whisper adds one lightweight Silero VAD pass, but neither backend adds another transcription pass or a fixed browser delay.
...
Service-metrics schema 11 reports this behaviour under `silero_endpoint_progress_publication_guard_v3` with numeric counts only. It records the guard strategy, speech-bounded input passes, total trailing silence trimmed, metadata-rejected and post-speech hypothesis counts, suppressed-word totals, word-timestamp passes, aligned-word totals, guarded edge words withheld/confirmed, actual recognition-pass intervals, cue processing, stable/mutable counts, cue lifetime, queue and Stop outcomes, translated draft/final publication counts, and first translated publication time measured from the first English cue update. Audio, recognition words, rejected wording, times, confidence values, captions, and translations remain excluded.
...
## Measuring Translation In v0.7.3
