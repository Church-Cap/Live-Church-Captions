...
Church Cap v0.7.3 retains up to five privacy-safe completed service summaries across app restarts and advances service metrics to schema 11. It adds text-free counts for speech-bounded inference inputs, total trailing silence trimmed, metadata-rejected and post-speech hypotheses while retaining word-alignment, guarded-edge, recognition-cadence, cue-latency, fair-queue, shutdown-drain, translated draft/final, and first translated-cue timing. Schema-9 and schema-10 histories remain readable. Use the separate anonymised service report for comparisons; use the broader diagnostics export only when support also needs computer and log information.
...
Run A/A2 on v0.7.3 before the multilingual matrix. Include several clear speak-then-pause sequences, a quieter passage, and normal room noise. Confirm that safe words remain prompt, legitimate final words remain visible, and no new unspoken wording appears after a clean pause. `source_units.engine` must be `word_timestamp_local_agreement_v5`, `interim_strategy` must be `immediate_stable_prefix_guarded_edge_tail`, and `transcription_streaming.guard_strategy` must be `silero_endpoint_progress_publication_guard_v3`; all Whisper runs should show non-zero `word_timestamp_passes` and `aligned_words`. During pauses, `speech_bounded_audio_passes` and `trailing_silence_trimmed_seconds` should become non-zero. A positive `edge_words_withheld` count can now include a weak suffix seen repeatedly on unchanged post-speech audio; `edge_words_confirmed` requires that the acoustic speech boundary advanced. Review `unreliable_hypotheses_suppressed`, `silence_hypotheses_suppressed`, and `suppressed_hypothesis_words` with the viewer observation: zero is valid when the model produces no suspect output, while a positive value proves an intervention without revealing wording. If transcription compute p95 is below the configured update interval, `transcription_streaming.pass_interval` p95 should remain close to that interval instead of interval plus compute time. Review edge-guard, cue, queue, recovery, and Stop measurements as before. The report proves scheduling and guard behaviour, not caption or translation quality.
